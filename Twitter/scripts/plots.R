...
print (retweetplot + geom_line(aes(colour=Affiliation)) + geom_point(aes(colour=Affiliation)) + facet_wrap(~Category, ncol=5) + 
         theme_bw() + scale_colour_manual(labels=c("Democratic", "Republican"), values=c("royalblue4","#9E2E2E")) + geom_ribbon(aes(ymin=CIlow, ymax=CIhigh), alpha=0.3) + ylab("Predicted Retweets") + labs(colour="Party") +
